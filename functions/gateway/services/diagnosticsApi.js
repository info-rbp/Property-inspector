...
const mockServiceCall = async (serviceName, endpoint) => {
...
exports.diagnosticsApi = {
    ...
    getServiceDetails: async (name) => {
        return mockServiceCall(name, 'ready');
    },
    // POST /api/diag/tests/run
    runSyntheticTest: async (type, updateCallback) => {
        ...
        const completeStep = (index, status, details) => {
    ...
    // POST /api/admin/ops
    performOp: async (action, payload) => {
        await DELAY(1000);
