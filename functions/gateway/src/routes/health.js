...
const healthRoutes = async (app) => {
    ...
    app.get('/ready', async (request, reply) => {
        ...
        const services = [
            { name: 'media-service', url: config_1.config.MEDIA_SERVICE_URL },
            { name: 'jobs-service', url: config_1.config.JOBS_SERVICE_URL },
            { name: 'billing-service', url: config_1.config.BILLING_SERVICE_URL }
        ];
    ...
    app.get('/version', async (request, reply) => {
        return {
            ...
            commit: config_1.config.GIT_COMMIT || 'unknown',
            buildTime: config_1.config.BUILD_TIME || 'unknown',
