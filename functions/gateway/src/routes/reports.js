...
const reportRoutes = async (app) => {
    ...
    app.post('/generate', async (request, reply) => {
        ...
        if (inspection.status === 'DRAFT') {
            throw new error_1.AppError(400, 'Cannot generate report for draft inspection');
        }
    ...
    app.get('/status/:jobId', async (request, reply) => {
        ...
        if (!job) {
            throw new error_1.AppError(404, 'Report job not found');
        }
        return {
            jobId: job.id,
            status: job.status,
    ...
    app.get('/download/:inspectionId', async (request, reply) => {
        ...
        if (!inspection.reportPath) {
            throw new error_1.AppError(404, 'Report not yet generated');
        }
        // In production, get signed URL from Media Service
        const downloadUrl = `${config_1.config.MEDIA_SERVICE_URL}/download/${inspection.reportMediaId}`;
        // Log audit event
    ...
    app.post('/finalize/:inspectionId', async (request, reply) => {
        ...
        if (inspection.reportStatus === 'FINALIZED') {
            throw new error_1.AppError(400, 'Report already finalized');
        }
        if (!inspection.reportPath) {
            throw new error_1.AppError(400, 'No report to finalize');
        }
