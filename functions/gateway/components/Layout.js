...
const Layout = ({ children, onNavigate, tenantName }) => {
    return (<div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      ...
      <aside className="w-full md:w-64 bg-slate-900 text-white flex-shrink-0">
        <div className="p-6 border-b border-slate-700">
          ...
          {tenantName && (<div className="mt-2 text-xs text-slate-400 uppercase tracking-wider truncate">
              {tenantName}
            </div>)}
        </div>

        <nav className="p-4 space-y-2">
          <button onClick={() => onNavigate('dashboard')} className="w-full flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg bg-slate-800 text-blue-400 hover:bg-slate-700 transition-colors">
            <lucide_react_1.LayoutDashboard size={18}/>
            Dashboard
          </button>
          <button className="w-full flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg text-slate-400 hover:bg-slate-800 hover:text-white transition-colors">
            <lucide_react_1.ClipboardList size={18}/>
            Inspections
          </button>
          <button className="w-full flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg text-slate-400 hover:bg-slate-800 hover:text-white transition-colors">
