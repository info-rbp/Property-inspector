...
const ProposalGenerator = () => {
    ...
    return (<div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in duration-500">
      ...
      <div className="mb-10">
        <div className="flex items-center justify-between relative max-w-2xl mx-auto">
           ...
           {[
            { id: 1, label: 'Client', icon: lucide_react_1.Building2 },
            { id: 2, label: 'Fees', icon: lucide_react_1.DollarSign },
            { id: 3, label: 'Terms', icon: lucide_react_1.PenTool },
            { id: 4, label: 'Review', icon: lucide_react_1.Check }
        ].map((s) => (<div key={s.id} className="flex flex-col items-center bg-gray-50 pt-2 px-2">
               <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold border-4 transition-all duration-300 shadow-sm ${step >= s.id
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8 min-h-[400px] flex flex-col justify-between">
        <div className="animate-in slide-in-from-right-4 fade-in duration-300">
          {step === 1 && (<div>
              <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
                <lucide_react_1.Building2 className="text-blue-600"/> Client & Property Details
              </h2>
          ...
          {step === 2 && (<div>
               <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
                 <lucide_react_1.DollarSign className="text-green-600"/> Services & Management Fees
               </h2>
          ...
          {step === 3 && (<div>
               <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
                 <lucide_react_1.PenTool className="text-purple-600"/> Custom Terms & Clauses
               </h2>
               <textarea className="w-full h-48 border border-gray-300 rounded-lg p-4 text-sm focus:ring-2 focus:ring-purple-500 outline-none" placeholder="Enter any specific terms, conditions or notes for this proposal..." defaultValue="The agency agrees to provide 4 routine inspections per annum."/>
             </div>)}
           {step === 4 && (<div className="text-center py-8">
               <div className="w-24 h-24 bg-green-50 rounded-full flex items-center justify-center mx-auto mb-6 text-green-600">
                  <lucide_react_1.FileOutput size={48}/>
               </div>
        ...
        <div className="flex justify-between mt-8 pt-6 border-t border-gray-100">
           <button onClick={() => setStep(s => Math.max(1, s - 1))} disabled={step === 1} className="px-6 py-2.5 rounded-lg border border-gray-300 text-gray-700 font-medium hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
             <lucide_react_1.ChevronLeft size={18}/> Back
           </button>
           <button onClick={() => setStep(s => Math.min(4, s + 1))} className={`px-8 py-2.5 rounded-lg text-white font-medium flex items-center gap-2 shadow-lg transition-transform active:scale-95 ${step === 4 ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'}`}>
             {step === 4 ? (<>Generate PDF <lucide_react_1.FileOutput size={18}/></>) : (<>Next Step <lucide_react_1.ChevronRight size={18}/></>)}
           </button>
        </div>
      </div>
    </div>);
};
exports.default = ProposalGenerator;
