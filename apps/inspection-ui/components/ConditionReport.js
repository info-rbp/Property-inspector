...
const ConditionReport = ({ reportType }) => {
    ...
    const [viewMode, setViewMode] = (0, react_2.useState)(types_2.ReportViewMode.EDIT);
    const [newRoomName, setNewRoomName] = (0, react_2.useState)('');
    // Handle document title for better "Save as PDF" filenames
    (0, react_2.useEffect)(() => {
        if (viewMode === types_2.ReportViewMode.PREVIEW) {
    ...
    if (viewMode === types_2.ReportViewMode.PREVIEW) {
        ...
        <div className="fixed top-4 right-4 flex gap-4 no-print z-50">
          ...
          <button onClick={() => setViewMode(types_2.ReportViewMode.EDIT)} className="bg-white hover:bg-gray-100 text-gray-800 px-4 py-2 rounded-lg shadow-lg flex items-center gap-2 font-medium">
    ...
    return (<div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in duration-500">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={() => setViewMode(types_2.ReportViewMode.PREVIEW)} className="bg-gray-900 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-800 transition flex items-center gap-2 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input type="text" value={report.propertyAddress} onChange={(e) => updateReport({ propertyAddress: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="e.g., 7 Riley St, Tuart Hill, WA 6060"/>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Client / Landlord</label>
              <input type="text" value={report.clientName} onChange={(e) => updateReport({ clientName: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="On behalf of..."/>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Inspecting Agent</label>
              <input type="text" value={report.agentName} onChange={(e) => updateReport({ agentName: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-blue-500 focus:outline-none"/>
            </div>
            <div>
            ...
            <div>
              ...
              <input type="text" value={report.tenantName} onChange={(e) => updateReport({ tenantName: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="e.g., John Doe"/>
            </div>
          </div>
        </div>
