...
const convertPhotosToBase64 = async (photos: Photo[]): Promise<any[]> => {
...
const callGeminiAPI = async (prompt: string, photos: Photo[]): Promise<string> => {
  try {
...
export const generateOverallComment = async (roomName: string, photos: Photo[], currentComment: string): Promise<string> => {
  const prompt = `
