...
import { Property, Inspection, InspectionItem, ConditionRating, InspectionStatus } from '../types';
import { ArrowLeft, Camera, CheckCircle, AlertTriangle, Save, ChevronRight, Key, User, Phone, Building } from 'lucide-react';
...
export const InspectionRunner: React.FC<InspectionRunnerProps> = ({ property, onClose }) => {
  const { dispatch } = useStore();
  const [step, setStep] = useState<'verify' | 'inspect' | 'review'>('verify');
  ...
  const [inspectionData, setInspectionData] = useState<Inspection>({
    ...
    items: ROOMS.map((room, idx) => ({
      ...
      category: 'Room',
      name: room,
      rating: ConditionRating.NOT_INSPECTED,
      notes: '',
      photos: []
    })),
    summary: ''
  ...
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>, itemId: string, currentPhotos: string[]) => {
  ...
  const progress = Math.round((inspectionData.items.filter(i => i.rating !== ConditionRating.NOT_INSPECTED).length / inspectionData.items.length) * 100);
    ...
    return (
      <div className="max-w-2xl mx-auto p-6 bg-white rounded-xl shadow-sm border border-slate-200 mt-10">
        ...
        <div className="mb-8 bg-blue-50 border border-blue-100 rounded-lg p-4 text-sm text-slate-700">
           <h3 className="font-bold text-blue-900 mb-2 flex items-center"><Building size={16} className="mr-2"/> Management Context</h3>
           <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                  ...
                  <div className="flex items-center">
                    <Key size={14} className="mr-1 opacity-70"/>
        ...
        <div className="flex justify-between items-center mb-6">
           ...
           <div className="flex items-center space-x-4">
              <button 
                 ...
                 className="text-slate-500 hover:text-blue-600 font-medium text-sm flex items-center"
              >
        ...
        <div className="flex-1 bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden flex flex-col">
           ...
           <div className="p-6 flex-1 overflow-y-auto">
             <div className="mb-6">
               ...
               <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                 {[ConditionRating.GOOD, ConditionRating.FAIR, ConditionRating.POOR, ConditionRating.CRITICAL].map((rating) => (
                   <button
                     key={rating}
                     onClick={() => updateItem(currentItem.id, { rating })}
                     className={`
             ...
             <div className="mb-6">
               ...
               <textarea 
                 ...
                 onChange={(e) => updateItem(currentItem.id, { notes: e.target.value })}
                 placeholder={`Describe condition of ${currentItem.name.toLowerCase()}...`}
             ...
             <div>
                ...
                <div className="flex items-center space-x-4 flex-wrap gap-y-4">
                  ...
                  {currentItem.photos.map((photo, i) => (
                    <img key={i} src={photo} alt="evidence" className="w-24 h-24 object-cover rounded-lg border border-slate-200" />
                  ))}
                </div>
             </div>
           </div>
           ...
           <div className="p-4 border-t border-slate-200 bg-slate-50 flex justify-between">
             <button 
               ...
               className="px-4 py-2 rounded text-slate-600 disabled:opacity-50"
             >
               Previous
             </button>
             <button 
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-xl shadow-lg border border-slate-200 mt-10">
      <div className="text-center mb-8">
        ...
        <p className="text-slate-500">You have rated {inspectionData.items.filter(i => i.rating !== ConditionRating.NOT_INSPECTED).length} areas.</p>
      </div>
      ...
      <div className="bg-slate-50 p-4 rounded-lg mb-6 max-h-60 overflow-y-auto">
        ...
        {inspectionData.items.filter(i => i.rating === ConditionRating.POOR || i.rating === ConditionRating.CRITICAL).length === 0 ? (
          <p className="text-sm text-slate-500 italic">No major defects recorded.</p>
        ) : (
          inspectionData.items.filter(i => i.rating === ConditionRating.POOR || i.rating === ConditionRating.CRITICAL).map(item => (
            <div key={item.id} className="flex items-start mb-2 last:mb-0">
               <AlertTriangle size={14} className="text-amber-500 mt-1 mr-2 flex-shrink-0" />
               <div>
