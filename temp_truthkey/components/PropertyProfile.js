...
const ConditionGauge_1 = require("./ConditionGauge");
const PropertyMap_1 = require("./PropertyMap");
const SourceBadge = ({ source }) => {
    let bg = 'bg-slate-100 text-slate-600';
    let icon = null;
    switch (source) {
        case types_2.DataSource.GNAF:
            bg = 'bg-blue-100 text-blue-800 border border-blue-200';
            icon = <lucide_react_1.CheckCircle2 size={10} className="mr-1"/>;
            break;
        case types_2.DataSource.LAND_REGISTRY:
        case types_2.DataSource.GOVT_PLANNING:
            bg = 'bg-slate-800 text-white border border-slate-700';
            icon = <lucide_react_1.Scale size={10} className="mr-1"/>;
            break;
        case types_2.DataSource.INSPECTOR:
            bg = 'bg-green-100 text-green-800 border border-green-200';
            icon = <lucide_react_1.CheckCircle2 size={10} className="mr-1"/>;
            break;
        case types_2.DataSource.LICENSED_PROVIDER:
            bg = 'bg-purple-50 text-purple-700 border border-purple-200';
            break;
        case types_2.DataSource.AI_PARSED:
            bg = 'bg-amber-50 text-amber-700 border border-amber-200 dashed border';
            break;
    }
    return (<span className={`inline-flex items-center text-[10px] font-bold px-2 py-0.5 rounded-full ${bg} ml-2 uppercase tracking-wide`}>
      {icon} {source}
    </span>);
};
const PropertyProfile = ({ property, inspections, onStartInspection, onUpdateField, onUpdateProperty }) => {
    const [activeTab, setActiveTab] = (0, react_2.useState)('identity');
    ...
    const reports = (0, react_2.useMemo)(() => {
        const list = [
            ...inspections
    ...
    const renderAttribute = (label, icon, fieldKey, field) => {
        if (!field)
            return null;
        return (<div className="flex flex-col p-4 bg-white border border-slate-200 rounded-lg shadow-sm hover:border-blue-400 transition-colors group">
        <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={() => {
                ...
                if (newValue !== null && newValue !== undefined && String(newValue).trim() !== '') {
                    ...
                    if (isNumber && Number.isNaN(nextValue))
                        return;
                    onUpdateField(fieldKey, {
                        ...field,
                        value: nextValue,
                        source: types_2.DataSource.USER,
    ...
    const renderLegalField = (label, value, fieldKey) => (<div className="bg-white p-3 rounded border border-slate-200 group hover:border-blue-400 transition-colors">
    ...
    return (<div className="max-w-6xl mx-auto p-4 md:p-6 space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm relative overflow-hidden flex flex-col md:flex-row">
        ...
        <div className="relative w-full md:w-1/3 h-64 md:h-auto bg-slate-200 group">
          <img src={property.thumbnailUrl || 'https://via.placeholder.com/400x300?text=No+Image'} alt={property.identity.address} className="w-full h-full object-cover"/>
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center">
             <button onClick={() => setIsEditingImage(!isEditingImage)} className="opacity-0 group-hover:opacity-100 bg-white text-slate-900 px-4 py-2 rounded-full font-bold text-sm shadow-lg flex items-center transition-all transform translate-y-2 group-hover:translate-y-0">
               <lucide_react_1.Camera size={16} className="mr-2"/>
               Change Photo
             </button>
          </div>
          
          {isEditingImage && (<div className="absolute top-16 left-1/2 -translate-x-1/2 w-64 bg-white rounded-xl shadow-xl p-4 z-20 border border-slate-200 animate-in fade-in zoom-in duration-200">
               ...
               <div className="space-y-2">
                 ...
                 <button onClick={handleImageUrl} className="flex items-center justify-center w-full px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-lg hover:bg-slate-50 text-sm font-medium transition-colors">
          ...
          <div className="flex flex-col h-full justify-between">
            ...
            <div className="mt-6 flex items-center justify-between">
               <div className="flex items-center space-x-4">
                 {latestInspection && <ConditionGauge_1.ConditionGauge inspection={latestInspection}/>}
                 <div>
               ...
               <button onClick={() => onStartInspection(property.id)} className="flex items-center bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 rounded-lg font-medium shadow-md transition-all active:scale-95">
                  <lucide_react_1.ClipboardCheck size={18} className="mr-2"/>
                  Inspect
                </button>
            </div>
          </div>
      ...
      <div className="border-b border-slate-200">
        <nav className="-mb-px flex space-x-8 overflow-x-auto">
          {[
            { id: 'identity', label: 'Identity & Legal', icon: <lucide_react_1.Scale size={16}/> },
            { id: 'structural', label: 'Structural', icon: <lucide_react_1.Home size={16}/> },
            { id: 'market', label: 'Market Context', icon: <lucide_react_1.BarChart3 size={16}/> },
            { id: 'history', label: 'History', icon: <lucide_react_1.History size={16}/> },
            { id: 'management', label: 'Management', icon: <lucide_react_1.Users size={16}/> },
            { id: 'reports', label: 'Reports', icon: <lucide_react_1.FileText size={16}/> },
      ...
      <div className="min-h-[400px]">
        ...
        {activeTab === 'identity' && (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-slate-50 p-6 rounded-xl border border-slate-200">
               <div className="flex items-center mb-4 text-slate-800">
                 <lucide_react_1.Database className="mr-2" size={20}/>
            ...
            <div className="bg-slate-50 p-6 rounded-xl border border-slate-200">
               <div className="flex items-center mb-4 text-slate-800">
                 <lucide_react_1.Scale className="mr-2" size={20}/>
             ...
             <div className="col-span-1 md:col-span-2 bg-white p-6 rounded-xl border border-slate-200">
                 <h3 className="flex items-center font-bold text-slate-800 mb-4">
                    <lucide_react_1.MapPin className="mr-2" size={18}/> Location Context
                 </h3>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="h-full min-h-[300px] relative group rounded-lg overflow-hidden border border-slate-200 shadow-inner">
                      <PropertyMap_1.PropertyMap lat={property.identity.latitude} lng={property.identity.longitude} title={property.identity.address}/>
                      {property.identity.latitude !== 0 && (<a href={`https://www.google.com/maps/search/?api=1&query=${property.identity.latitude},${property.identity.longitude}`} target="_blank" rel="noreferrer" className="absolute bottom-2 right-2 bg-white/90 hover:bg-white text-slate-600 hover:text-blue-600 text-xs px-2 py-1 rounded shadow-sm border border-slate-200 flex items-center transition-colors opacity-0 group-hover:opacity-100 z-10">
                          <lucide_react_1.ExternalLink size={10} className="mr-1"/>
                          View on Google Maps
                        </a>)}
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                           ...
                           <ul className="text-sm text-slate-700 space-y-1">
                              {property.location.schools.map((s, i) => (<li key={i}>{s.name} <span className="text-slate-400">({s.distanceKm}km)</span></li>))}
                           </ul>
                        </div>
                        <div>
                           <h4 className="text-xs font-bold text-slate-500 uppercase mb-2">Transport</h4>
                           <ul className="text-sm text-slate-700 space-y-1">
                              {property.location.transport.map((s, i) => (<li key={i}>{s.name} <span className="text-slate-400">({s.distanceKm}km)</span></li>))}
                           </ul>
                        </div>
                        <div className="md:col-span-2">
                           <h4 className="text-xs font-bold text-slate-500 uppercase mb-2">Risk Factors</h4>
                           <ul className="text-sm text-slate-700 space-y-1">
                              {property.location.emergencyServices.map((s, i) => (<li key={i} className="flex items-center justify-between">
                                   {s.type} 
        ...
        {activeTab === 'structural' && (<div className="space-y-6">
            <div className="flex items-center p-4 bg-blue-50 text-blue-800 rounded-lg border border-blue-100 text-sm">
               <lucide_react_1.ShieldAlert className="mr-2" size={18}/>
               These attributes are pre-filled from external data. They must be verified by an inspector.
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
               {renderAttribute("Property Type", <lucide_react_1.Building size={14}/>, 'type', property.attributes.type)}
               {renderAttribute("Bedrooms", <lucide_react_1.BedDouble size={14}/>, 'bedrooms', property.attributes.bedrooms)}
               {renderAttribute("Bathrooms", <lucide_react_1.Bath size={14}/>, 'bathrooms', property.attributes.bathrooms)}
               {renderAttribute("Car Spaces", <lucide_react_1.Car size={14}/>, 'carSpaces', property.attributes.carSpaces)}
               {renderAttribute("Year Built", <lucide_react_1.Calendar size={14}/>, 'yearBuilt', property.attributes.yearBuilt)}
               {renderAttribute("Floor Area (m²)", <lucide_react_1.Building size={14}/>, 'floorAreaSqm', property.attributes.floorAreaSqm)}
            </div>
          </div>)}

        {/* TAB 3: MARKET CONTEXT (Aggregates) */}
        {activeTab === 'market' && (<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="h-[300px] w-full">
                <recharts_1.ResponsiveContainer width="100%" height="100%">
                  <recharts_1.AreaChart data={trendData}>
                    ...
                    <recharts_1.XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: '#64748b' }}/>
                    <recharts_1.YAxis axisLine={false} tickLine={false} tick={{ fill: '#64748b' }} domain={['dataMin - 50', 'dataMax + 50']}/>
                    <recharts_1.Tooltip contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}/>
                    <recharts_1.Area type="monotone" dataKey="uv" stroke="#3b82f6" strokeWidth={3} fillOpacity={1} fill="url(#colorUv)"/>
              ...
              <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                 ...
                 <div className="flex items-center justify-between">
                   ...
                   <span className={`flex items-center text-sm font-bold ${property.rentalContext.trend12Month === 'up' ? 'text-green-600' : 'text-red-600'}`}>
                     {property.rentalContext.trend12Month === 'up' ? <lucide_react_1.TrendingUp size={14} className="mr-1"/> : <lucide_react_1.TrendingDown size={14} className="mr-1"/>}
        ...
        {activeTab === 'history' && (<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
             <div className="bg-white p-6 rounded-xl border border-slate-200">
                <h3 className="flex items-center font-bold text-slate-800 mb-4">
                   <lucide_react_1.DollarSign className="mr-2" size={18}/> Sales History
                </h3>
                <div className="space-y-4 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 before:to-transparent">
                  {property.salesHistory.map((sale, i) => (<div key={i} className="relative flex items-center justify-between pl-8 group">
             ...
             <div className="space-y-4">
                <h3 className="flex items-center font-bold text-slate-800">
                   <lucide_react_1.ClipboardCheck className="mr-2" size={18}/> Inspection Log
                </h3>
                {inspections.length === 0 ? (<div className="text-center py-12 text-slate-400 bg-slate-50 rounded-lg border border-dashed border-slate-200">No inspections recorded.</div>) : (inspections.map(insp => (<div key={insp.id} className="flex items-center bg-white p-4 rounded-lg border border-slate-200 hover:shadow-md transition-shadow">
        ...
        {activeTab === 'management' && (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                <h3 className="flex items-center font-bold text-slate-800 mb-6">
                   <lucide_react_1.Users className="mr-2" size={20}/> Ownership & Management
                </h3>
                <div className="space-y-6">
                   <div>
                      ...
                      {property.management?.managedBy === types_2.ManagedByType.REAL_ESTATE_AGENCY && property.management.managingAgencyName && (<div className="text-sm text-slate-600 mt-1">{property.management.managingAgencyName}</div>)}
                   </div>
             ...
             <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                <h3 className="flex items-center font-bold text-slate-800 mb-6">
                   <lucide_react_1.Key className="mr-2" size={20}/> Access Control
                </h3>
                <div className="space-y-6">
                   <div>
        ...
        {/* TAB 6: REPORTS */}
        {activeTab === 'reports' && (<div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-200 flex justify-between items-center">
              <div>
                <h3 className="flex items-center font-bold text-slate-800 text-lg">
                   <lucide_react_1.FileText className="mr-2" size={20}/> Property Reports
                </h3>
            ...
            <div className="divide-y divide-slate-100">
              {reports.length === 0 ? (<div className="p-12 text-center text-slate-400 italic">No reports generated yet.</div>) : (reports.map((report) => (<div key={report.id} className="p-4 hover:bg-slate-50 flex items-center justify-between transition-colors group">
                    <div className="flex items-center space-x-4">
