...
import { Issue, IssueSource, IssueSeverity, AiIssueResolution, InspectionStatus } from '../types';
import { Check, X, Edit2, AlertTriangle, User, Bot, Trash2 } from 'lucide-react';
...
export const IssueManager: React.FC<IssueManagerProps> = ({ issues, componentId, isLocked, onRefresh }) => {
  // 1. Separation Logic
  const humanIssues = issues.filter(i => i.source === IssueSource.HUMAN);
  const aiIssues = issues.filter(i => i.source === IssueSource.AI && (!i.ai_resolution || i.ai_resolution === AiIssueResolution.PENDING));
  const rejectedAiIssues = issues.filter(i => i.ai_resolution === AiIssueResolution.REJECTED);

  const [isEditing, setIsEditing] = useState<string | null>(null); // 'new' or issueId
  const [editForm, setEditForm] = useState<Partial<Issue>>({});
  const [showRejected, setShowRejected] = useState(false);

  const handleCreateHumanIssue = async () => {
    try {
      await gatewayService.createHumanIssue(componentId, {
        type: editForm.type || 'observation',
        severity: editForm.severity || IssueSeverity.MINOR,
        notes: editForm.notes
      });
      setIsEditing(null);
      setEditForm({});
      onRefresh();
    } catch (e) { alert("Failed to add issue"); }
  };

  const handleAcceptAi = async (aiIssue: Issue) => {
    try {
      await gatewayService.resolveAiIssue(aiIssue.issue_id, 'accept');
      onRefresh();
    } catch (e) { alert("Failed to accept"); }
  };

  const handleRejectAi = async (aiIssue: Issue) => {
    if (!confirm("Reject this suggestion? It will be hidden from reports.")) return;
    try {
      await gatewayService.resolveAiIssue(aiIssue.issue_id, 'reject');
      onRefresh();
    } catch (e) { alert("Failed to reject"); }
  };

  const handleOverrideAi = async (aiIssue: Issue) => {
  ...
  const submitOverride = async (aiIssueId: string) => {
    try {
      await gatewayService.resolveAiIssue(aiIssueId, 'accept', editForm);
      setIsEditing(null);
      setEditForm({});
      onRefresh();
    } catch (e) { alert("Failed to override"); }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <div className="flex justify-between items-center mb-3">
          <h4 className="text-sm font-bold text-gray-800 flex items-center gap-2">
            <User size={16} className="text-blue-600" /> Human Findings
            <span className="bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full text-xs">{humanIssues.length}</span>
          </h4>
          {!isLocked && (
            <button 
              onClick={() => setIsEditing('new')}
              className="text-xs bg-blue-50 text-blue-700 px-3 py-1.5 rounded-md hover:bg-blue-100 font-medium"
            >
              + Add Issue
            </button>
          )}
        ...
        <div className="space-y-3">
          {humanIssues.map(issue => (
            <div key={issue.issue_id} className="bg-white border border-l-4 border-l-blue-500 border-gray-200 p-3 rounded shadow-sm">
              <div className="flex justify-between">
                ...
                <span className={`text-[10px] px-2 py-0.5 rounded uppercase font-bold tracking-wide ${
                   ...
                   issue.severity === IssueSeverity.MAJOR ? 'bg-orange-100 text-orange-700' : 
                   'bg-gray-100 text-gray-600'
                }`}>{issue.severity}</span>
              </div>
              <p className="text-sm text-gray-600 mt-1">{issue.notes}</p>
              {issue.provenance?.derived_from_issue_id && (
                <div className="mt-2 text-xs text-blue-400 flex items-center gap-1">
                  <Bot size={10} /> Copied from AI suggestion
                </div>
              )}
            </div>
          ))}
      ...
      <div className="border-t border-gray-200 pt-4">
        ...
        <div className="space-y-3">
          {aiIssues.map(issue => (
            isEditing === issue.issue_id ? (
              <div key={issue.issue_id} className="border border-purple-200 rounded p-2 bg-purple-50">
                 <p className="text-xs text-purple-700 mb-2 font-medium">Overriding AI Suggestion:</p>
                 <IssueEditor 
                   initialData={{ type: issue.type, severity: issue.severity, notes: issue.notes }} 
                   onSave={() => submitOverride(issue.issue_id)} 
                   onCancel={() => setIsEditing(null)} 
                 />
              </div>
            ) : (
              <div key={issue.issue_id} className="bg-purple-50 border border-purple-100 p-3 rounded hover:border-purple-300 transition-colors">
        ...
        <div className="border-t border-gray-200 pt-2">
           ...
           {showRejected && (
              <div className="mt-2 space-y-2 opacity-60">
                {rejectedAiIssues.map(i => (
                  <div key={i.issue_id} className="text-xs text-gray-500 pl-2 border-l-2 border-gray-300">
...
const IssueEditor: React.FC<{ initialData: any, onSave: (data: any) => void, onCancel: () => void }> = ({ initialData, onSave, onCancel }) => {
  ...
  return (
    <div className="bg-gray-50 p-3 rounded border border-gray-200 space-y-3">
       ...
       <div>
         ...
         <select 
            className="w-full text-sm p-1.5 rounded border"
            value={formData.severity || IssueSeverity.MINOR}
