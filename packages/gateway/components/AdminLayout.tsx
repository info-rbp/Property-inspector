...
import { 
  Activity, Server, Shield, Database, LayoutGrid, Users, Settings, LogOut, Terminal 
} from 'lucide-react';
...
export const AdminLayout: React.FC<AdminLayoutProps> = ({ children, currentView, onNavigate }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 flex flex-col md:flex-row font-mono">
      {/* Sidebar */}
      <aside className="w-full md:w-64 bg-slate-900 border-r border-slate-800 flex-shrink-0 flex flex-col">
        ...
        <nav className="p-4 space-y-1 flex-1">
          ...
          <NavItem view="dashboard" icon={LayoutGrid} label="Service Overview" />
          <NavItem view="tests" icon={Activity} label="Synthetic Tests" />
          
          <div className="text-[10px] text-slate-600 font-bold uppercase tracking-wider px-4 mb-2 mt-6">Administration</div>
          <NavItem view="tenants" icon={Users} label="Tenant Management" />
          <NavItem view="ops" icon={Settings} label="Ops Actions" />
        </nav>
