...
class TemplateService {
    async render(templateId, variables) {
        // 1. Fetch Template
        const res = await db_1.db.query(`SELECT subject_template, body_template, version 
       ...
       ORDER BY version DESC LIMIT 1`, [templateId]);
        ...
        const subjectCompiled = handlebars_1.default.compile(subject_template);
        const bodyCompiled = handlebars_1.default.compile(body_template);
        const subject = subjectCompiled(variables);
        const bodyMjml = bodyCompiled(variables);
        // 3. Render MJML to HTML
        const mjmlResult = (0, mjml_1.default)(bodyMjml);
