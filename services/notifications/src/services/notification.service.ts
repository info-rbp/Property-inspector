...
const emailProvider = new SmtpProvider(); // In a real app, use a factory
...
interface EnqueueParams {
  ...
  triggeredBy: { actorType: string; actorId?: string };
  variables: any;
}
...
export class NotificationService {
  ...
  async enqueue(params: EnqueueParams) {
    ...
    try {
      ...
      const existingRes = await client.query(
        `SELECT notification_id, status FROM notifications WHERE tenant_id = $1 AND idempotency_key = $2`,
   ...
   */
  async preview(params: { tenantId: string; templateId: string; variables: any }) {
    // 1. Fetch Branding
   ...
   * Worker Logic
   */
  async processDelivery(notificationId: string) {
    const res = await db.query(`SELECT * FROM notifications WHERE notification_id = $1`, [notificationId]);
    if (res.rows.length === 0) return; // Should not happen

    const notification = res.rows[0];

    // Terminal states check
    if (['SENT', 'CANCELLED', 'DEAD_LETTER'].includes(notification.status)) {
      logger.info('Notification already terminal', { id: notificationId, status: notification.status });
      return;
    }

    // Check attempts
    if (notification.attempts >= config.logic.maxAttempts) {
      await db.query(`UPDATE notifications SET status = 'DEAD_LETTER' WHERE notification_id = $1`, [notificationId]);
      logger.warn('Notification moved to DEAD_LETTER', { id: notificationId });
      return;
    }

    // Mark SENDING
    await db.query(`UPDATE notifications SET status = 'SENDING', attempts = attempts + 1, updated_at = NOW() WHERE notification_id = $1`, [notificationId]);
    ...
    try {
      ...
      await db.query(
        `UPDATE notifications SET status = 'FAILED', last_error = $1 WHERE notification_id = $2`,
        [JSON.stringify(errorData), notificationId]
      );

      // Re-enqueue if not maxed out
      if (notification.attempts + 1 < config.logic.maxAttempts) {
        logger.info(`Scheduling retry for ${notificationId} in ${backoffSeconds}s`);
      ...
      // 6. Record Attempt
      await db.query(
        `INSERT INTO notification_attempts (notification_id, attempt_number, status, provider_response, error, started_at, finished_at)
  ...
  async getStatus(notificationId: string, tenantId: string) {
    const res = await db.query(
      `SELECT notification_id, status, created_at, sent_at, last_error FROM notifications WHERE notification_id = $1 AND tenant_id = $2`,
