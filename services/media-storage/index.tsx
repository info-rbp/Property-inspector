...
const AIStudio = () => {
  const [activeSubTab, setActiveSubTab] = useState<'chat' | 'image' | 'vision' | 'search'>('chat');
...
const ChatModule = () => {
  ...
  const [mode, setMode] = useState<'pro' | 'fast' | 'thinking'>('pro');
  ...
  return (
    <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div className="md:col-span-2 bg-gray-100 rounded-lg flex items-center justify-center min-h-[400px] border-2 border-dashed border-gray-300">
        {generatedImage ? (
          <img src={generatedImage} alt="Generated" className="max-w-full max-h-[600px] object-contain rounded shadow-lg" />
        ) : (
...
const VisionModule = () => {
  ...
  const [prompt, setPrompt] = useState('Analyze this image and list the key elements.');
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="space-y-4">
           <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:bg-gray-50 transition-colors cursor-pointer relative">
             ...
             />
             {preview ? (
               <img src={preview} alt="Upload" className="max-h-64 mx-auto rounded" />
             ) : (
        ...
        <div className="bg-gray-50 rounded-lg p-6 border border-gray-200 min-h-[300px]">
          ...
          {result ? (
            <div className="whitespace-pre-wrap text-gray-800 text-sm">{result}</div>
          ) : (
            <div className="text-gray-400 text-sm italic">Results will appear here...</div>
          )}
        </div>
      </div>
    </div>
...
const SearchModule = () => {
  ...
  const [type, setType] = useState<'web' | 'maps'>('web');
  const [result, setResult] = useState('');
  ...
  const search = async () => {
    ...
    try {
      ...
      const extractedSources = chunks
        .map((c: any) => {
          if (c.web) return { title: c.web.title, uri: c.web.uri, type: 'web' };
          if (c.maps) return { title: c.maps.title, uri: c.maps.uri, type: 'maps' };
          return null;
        })
        .filter(Boolean);
      ...
      <div className="flex gap-4">
        <div className="flex-1 relative">
          <input 
            ...
            placeholder={type === 'web' ? "Search for latest news..." : "Find places nearby..."}
            className="w-full border border-gray-300 rounded-lg pl-4 pr-12 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
          <div className="absolute right-2 top-2 flex bg-gray-100 rounded p-0.5">
             <button 
               onClick={() => setType('web')}
               className={`px-2 py-1 text-xs rounded ${type === 'web' ? 'bg-white shadow text-indigo-600' : 'text-gray-500'}`}
             >
               Web
             </button>
             <button 
               onClick={() => setType('maps')}
               className={`px-2 py-1 text-xs rounded ${type === 'maps' ? 'bg-white shadow text-indigo-600' : 'text-gray-500'}`}
             >
               Maps
             </button>
          </div>
    ...
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans">
      ...
      <main className="max-w-5xl mx-auto p-6">
        <div className="flex space-x-4 mb-6 border-b border-gray-200 pb-2 overflow-x-auto">
          {['overview', 'api', 'ai-studio', 'architecture', 'client-helper'].map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-4 py-2 capitalize font-medium whitespace-nowrap ${
